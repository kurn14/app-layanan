...
class StatusHistoriesRelationManager extends RelationManager
{
    ...
    protected static ?string $title = 'Riwayat Status Penanganan';
        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('from_status')
                    ->label('Dari Status')
                    ->badge()
                    ->formatStateUsing(fn ($state) => $state ? (ComplaintStatus::tryFrom((string) $state)?->label() ?? $state) : 'Laporan Baru')
                    ->color('gray'),
                TextColumn::make('to_status')
                    ->label('Menjadi Status')
                    ->badge()
                    ->formatStateUsing(fn ($state) => ComplaintStatus::tryFrom((string) $state)?->label() ?? $state)
                    ->color('info'),
